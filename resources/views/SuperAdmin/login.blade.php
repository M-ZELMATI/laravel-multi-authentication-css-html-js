...
<body>
    <div class="container">
        ...
        <form method="POST" action="{{ route('superadmin.login') }}" id='upload_form'  enctype="multipart/form-data">
        <!-- action="{{ route('login') }}"  -->
            @csrf
        ...
        <button  name="upload" id="uploadsuperAdminlogin"  data-token="{{ csrf_token() }}"  >Sign in</button>
        <p class="join-link">
            New to linkedin?
